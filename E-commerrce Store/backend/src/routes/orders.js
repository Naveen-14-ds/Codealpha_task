...
router.get("/", (req, res) => {
  const orders = db
    .prepare(
      `SELECT id, user_id, total, status, shipping_name, shipping_address, shipping_phone, created_at FROM orders WHERE user_id = ? ORDER BY id DESC`
    )
    .all(req.userId);
...
router.post("/", (req, res) => {
  ...
  try {
    ...
    const orderInfo = db
      .prepare(
        `INSERT INTO orders (user_id, total, status, shipping_name, shipping_address, shipping_phone) VALUES (?, ?, 'placed', ?, ?, ?)`
      )
      .run(
        req.userId,
  ...
  const order = db
    .prepare(`SELECT id, user_id, total, status, shipping_name, shipping_address, shipping_phone, created_at FROM orders WHERE id = ?`)
    .get(orderId);
  const items = db
    .prepare(
