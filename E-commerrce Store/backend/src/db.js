...
// Add shipping columns if they don't exist (safe to run every start)
try {
  (function ensureOrderShippingColumns() {
    const cols = db
      .prepare("PRAGMA table_info('orders')")
      .all()
      .map((r) => r.name);
    const toAdd = [
      { name: "shipping_name", def: "TEXT DEFAULT ''" },
      { name: "shipping_address", def: "TEXT DEFAULT ''" },
      { name: "shipping_phone", def: "TEXT DEFAULT ''" },
    ];
    for (const c of toAdd) {
      if (!cols.includes(c.name)) {
        db.exec(`ALTER TABLE orders ADD COLUMN ${c.name} ${c.def}`);
      }
    }
  })();
} catch (err) {
  console.error('Failed to ensure shipping columns:', err);
}
